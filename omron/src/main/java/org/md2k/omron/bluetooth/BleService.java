					...
					if (mCtsNeedToWrite) {
						...
						if (ret) {
							String date = year
									+ "/" + data[2]
									+ "/" + data[3]
									+ " " + String.format(Locale.US, "%1$02d", data[4])
									+ ":" + String.format(Locale.US, "%1$02d", data[5])
									+ ":" + String.format(Locale.US, "%1$02d", data[6])
									+ " (WeekOfDay:" + data[7]
									+ " Fractions256:" + data[8]
									+ " AdjustReason:" + data[9] + ")";
	...
	private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(BluetoothDevice.ACTION_BOND_STATE_CHANGED)) {
				...
				Log.i(TAG, "[LOG]ACTION_BOND_STATE_CHANGED: " + String.format(Locale.US, "bond_state prev=%d, now=%d", prev_bond_state, bond_state));
